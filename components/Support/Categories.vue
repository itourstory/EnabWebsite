    ...
    <div class="row mx-lg-n4">
      <!-- Get Started -->
        <div v-for="category in categories" :key="category.id"  class="col-lg-4 col-md-6 px-lg-4">
          <nuxt-link :to="pagesLinks[category.id-1]">
...
export default {
  ...
  computed:{
    ...
    pagesLinks(){
      return ['/support','/support','/support','/docs','/support','/support']//page number 4 for category number 4 => Services page (/docs)
    }
  },
  methods:{
    send_id(tag_id){
      this.$emit('clicked',tag_id)
      console.log(tag_id)
    }
  }
}
